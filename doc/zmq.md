...
The TitaniumCoin Core daemon can be configured to act as a trusted "border
router", implementing the titaniumcoin wire protocol and relay, making
...
The ZeroMQ feature in TitaniumCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building titaniumcoind:
...
For instance:
    ...
    $ titaniumcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/titaniumcoind.tx.raw \
...
These options can also be provided in titaniumcoin.conf.
...
From the perspective of titaniumcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into titaniumcoind directly. Furthermore, no information is
...
using. TitaniumCoind appends an up-counting sequence number to each
