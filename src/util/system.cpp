...
const char * const BITCOIN_CONF_FILENAME = "titaniumcoin.conf";
...
#else
    const char* pszModule = "titaniumcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TitaniumCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/TitaniumCoin";
#else
    // Unix-like
    return pathRet / ".titaniumcoin";
#endif
#endif
}
